...
def test_init_db_creates_chuck_file_change_tables():
    """init_db creates module-owned file changer queue tables."""
    mock_conn, mock_cursor = _make_mock_conn()
    with patch("pymysql.connections.Connection", return_value=mock_conn):
        import toolsdb
        ...
        toolsdb.init_db()
    executed = " ".join(str(c) for c in mock_cursor.execute.call_args_list)
    assert "chuck_file_change_jobs" in executed
    assert "chuck_file_change_job_items" in executed
...
def test_init_db_sets_item_status_default_and_attempts_column():
    ...
    with patch("pymysql.connections.Connection", return_value=mock_conn):
        ...
        toolsdb.init_db()
    executed = " ".join(str(c) for c in mock_cursor.execute.call_args_list)
    assert "DEFAULT 'queued'" in executed
