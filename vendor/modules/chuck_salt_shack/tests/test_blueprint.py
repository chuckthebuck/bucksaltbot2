...
def test_saltlick_rights_are_generated_and_enforced_independently(monkeypatch):
    test_client = client(monkeypatch)
    monkeypatch.setattr(
        api,
        "_has_right",
        lambda _username, right: right == "saltlick_page_purger_preview",
    )
    monkeypatch.setattr(api, "saltlick_is_enabled", lambda _saltlick_id: True)
    monkeypatch.setattr(api, "_enqueue_saltlick", lambda *_args, **_kwargs: 88)

    response = test_client.post(
        "/api/v1/modules/chuck_salt_shack/saltlicks/page_purger/runs",
        json={
            "mode": "preview",
            "inputs": {"targets": [{"title": "Main Page", "namespace": 0}]},
        },
    )

    assert response.status_code == 202
    catalog = test_client.get("/api/v1/modules/chuck_salt_shack/saltlicks")
    page_purger = next(
        item for item in catalog.get_json()["saltlicks"] if item["id"] == "page_purger"
    )
    assert page_purger["rights"] == {
        "preview": "saltlick_page_purger_preview",
        "apply": "saltlick_page_purger_apply",
        "estop": "saltlick_page_purger_estop",
    }


def test_stopped_saltlick_cannot_be_queued(monkeypatch):
    monkeypatch.setattr(api, "saltlick_is_enabled", lambda _saltlick_id: False)
    ...
    assert "emergency-stopped" in response.get_json()["detail"]
