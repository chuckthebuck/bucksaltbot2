...
onMounted(async () => {
  try {
    ...
    const [authBody, registryBody] = await Promise.all([
      requestJson("/api/v1/modules/saltlick/auth"),
      requestJson("/api/v1/modules/saltlick/saltlicks"),
    ]);
    auth.value = authBody;
...
async function loadHistory() {
  try {
    const body = await requestJson(
      `/api/v1/modules/saltlick/saltlicks/${encodeURIComponent(selectedId.value)}/runs`,
    );
...
async function startRun(mode: "preview" | "apply") {
  ...
  try {
    ...
    const body = await requestJson(
      `/api/v1/modules/saltlick/saltlicks/${encodeURIComponent(contract.id)}/runs`,
      {
        method: "POST",
...
async function pollRun(id: number) {
  for (let attempt = 0; attempt < 360; attempt += 1) {
    const body = await requestJson(`/api/v1/modules/saltlick/runs/${id}`);
...
<template>
  <main class="salt-shack">
    ...
    <cdx-message v-else-if="!loading && !saltlicks.length" type="warning">
